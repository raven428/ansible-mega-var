...
def main() -> None:  # noqa: C901
  ...
  if not f_name.endswith('/'):
    if f_dest and f_dest.endswith('/'):
      dest4path = prefix + re.sub(r'/+$', '', f_dest) + '/' + Path(f_name).name
    elif f_dest:
  ...
  # dest is created
  if dest4copy.endswith('/') or src_path.is_dir():
    dest4dir = ''
